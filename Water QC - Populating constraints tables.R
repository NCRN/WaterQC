...
low <- wqdata %>%
  ...
  summarise(low=quantile(ResultMeasureValue,0.10,na.rm=T))

high <- wqdata %>%
  group_by(MonitoringLocationIdentifier) %>%
  group_by(CharacteristicName, .add=T) %>%
  summarise(high=quantile(ResultMeasureValue,0.90,na.rm=T))
...
################################################################################
### Identify low and high values (by month) ####################################
################################################################################
...
# Name fields
low_cn$SoftConstraints_name[1] <- "MonitoringLocationIdentifier"
low_cn$SoftConstraints_name[2] <- "Month"
high_cn$SoftConstraints_name[1] <- "MonitoringLocationIdentifier"
high_cn$SoftConstraints_name[2] <- "Month"
...
# Join together
lowHigh <- left_join(pw_low, pw_high, by=c("MonitoringLocationIdentifier","Month"))
colnames(lowHigh)[1] <- "Location_ID"
...
# Add month column to s_con
s_con2 <- add_column(s_con, Month=NA,.before=2)

# Sort columns
sorted <- lowHigh[,colnames(s_con2)]
